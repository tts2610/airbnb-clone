...
exports.postReviews = async(req, res, next) => {
    ...
    try {
        ...
        const exp = await Experience.findOne({ _id: mongo.Types.ObjectId(experienceID) })
            // const exp = await Experience.find({ _id: mongo.Types.ObjectId(experienceID) })
        ...
        let review = await Review.create({ comments: comments, userId: user, rating: rating, experienceId: exp })
