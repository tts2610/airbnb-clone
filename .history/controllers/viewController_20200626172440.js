...
exports.postReviews = async(req, res, next) => {
    ...
    const user = req.user;
    const experience = req.headers.expID;


    res.status(200).json({ status: "post successfully", data: experience })
    if (!experience) {
        res.status(500).json({ status: "failed", data: "Please choose a comment to post comment!" })
    } else {
        res.status(200).json({ status: "post successfully", data: experience })
    }

    // let review = await Review.create({ comments: comments, userId: user, rating: rating })
