...
exports.updateExp = async(req, res, next) => {
    ...
    if (!exp) {
        ...
        if (tags) {
            let newTags = await Tag.convertToObject(tags);
            updatedExp = await Experience.findByIdAndUpdate(id, { title, images, location, rating, tags: newTags });
        } else {
            updatedExp = await Experience.findByIdAndUpdate(id, { title, images, location, rating });
        }
